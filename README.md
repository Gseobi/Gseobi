...
실제 서비스 운영 환경에서  
결제(PG) 연동, 장기 실행 Daemon, Scheduler, 지연 딥링크(Deferred Deep Link) 등  
**운영 민감도가 높은 백엔드 시스템을 직접 설계·구현·운영**해왔습니다.

- Java / Spring 기반 Backend API 설계·개발·운영
- 다수 PG사를 연동하는 **결제 분기 Gateway 구조 설계·개발·운영**
  - 해당 경험을 바탕으로 Wallet 서비스 제공 협력사에 결제 서비스 구현 기술 지원 경험
- Socket, HTTP/HTTPS, SMTP, FCM Push 기반 **장기 실행 Daemon 아키텍처 설계·개발·운영**
  - Pure Java Base / Spring Boot Base 모두 개발 가능
- 이중화 환경에서의 Play Store / App Store 리뷰 조회 서버  
  **Scheduler 중복 실행 방지 구조 설계·개발·운영**
- **Deferred Deep Link 백엔드 프로세스 및 아키텍처 전담 설계**
  - Click ID 기반 트래킹
  - 암호화 토큰 기반 위·변조 방지
  - 설치 / 최초 실행 시점 검증 로직 구현
- MVNO 조회·충전 및 PIN(쿠폰/기프티콘) 발급·충전·조회 시스템 연동 경험
- 외부 업체 및 Open API, Socket, Servlet 기반 시스템 연동 다수 경험

### 개발 모토
**“운영 중 장애가 발생하면 즉시 영향을 주는 영역”**을 중심으로  
문제 정의 → 구조 설계 → 구현 → 운영 대응까지 경험해왔습니다.

단순 기능 구현보다는 아래 항목을 우선 고려합니다.
...
- **image-synthesis-multi-thread**
  - 비동기로 이미지 합성 후 DB와 CDN서버에 저장하는 프로젝트의 재구성
- **java-daemon-collection**
  - HTTP / HTTPS / SMTP / FCM Push 기반 Daemon 시리즈 (Pure Java & Spring)
- **java-util-collection**
  - 형변환, JSON Build·Parsing, Encrypt·Decrypt, HTTP/HTTPS 통신 등의 통합 Util
